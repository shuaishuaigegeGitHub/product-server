...
router.get('/findProductTaskAll', async (ctx) => {
    ctx.body = await service.findProductTaskAll(ctx.request.query, ctx.request.headers.token);
});
/**
 * 按照分组查询任务
 */
router.get('/findGroupTask', async (ctx) => {
    ctx.body = await service.findGroupTask(ctx.request.query, ctx.request.headers.token);
});
/**
 * 查询任务详情
 */
router.get('/findTaskDetail', async (ctx) => {
    ctx.body = await service.findTaskDetail(ctx.request.query, ctx.request.headers.token);
});
/**
 * 完成任务
 */
