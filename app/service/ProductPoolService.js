        ...
        await models.product.update(
            {
                ...
                approval_end_time: param.approval_end_time
                    ? parseInt(param.approval_end_time / 1000)
                    : 0,
                status: 2,
                approval_time: time
            },
...
export const assessment = async param => {
    ...
    try {
        if (status == 3) {
            ...
            await models.task_group.bulkCreate(bulk, transaction);
        }
        // 更新主表
        await models.product.update(
            {
        ...
        await models.product_schedule.update(
            {
                ...
                estimate_program_person:
                    param.estimate_program_person || undefined,
                ...
                procedure_evaluation: JSON.stringify(
                    param.procedure_evaluation
                ),
                art_evaluation: JSON.stringify(param.art_evaluation),
                operational_evaluation: JSON.stringify(
                    param.operational_evaluation
                ),
                ...
                wide_electric_approval:
                    param.wide_electric_approval || undefined
            },
            {
                where: {
                    product_id: param.id
...
export const findAll = async (param, token, headerTOken) => {
    ...
    let sqlAll =
        ' select count(1) as num from product t1 left join product_base t2 on t1.id=t2.product_id ';
    let object = {
            'game_type$=': param.game_type,
...
export const findDetail = async param => {
    ...
    if (result && result.length) {
        ...
        if (data.operational_evaluation && data.operational_evaluation.length) {
            data.operational_evaluation = JSON.parse(
                data.operational_evaluation
            );
        }
...
export const putIntoProduction = async param => {
    ...
    await models.task_group.bulkCreate(bulk);
...
function assessmentCheck(param) {
    ...
    if (
        !param.estimate_program_person ||
        isNaN(param.estimate_program_person)
    ) {
