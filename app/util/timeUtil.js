...
// 时间戳（秒）转字符串时间
export const unixToStr = (unix, format = NORMAL_TIME_FORMAT) => dayjs.unix(unix).format(format);
...
export const objTimeFormater = (obj, options = {}) => {
    ...
    // 是否在当前实例上操作
    const cur = (options.cur === undefined || options.cur === null) ? true : options.cur;
 ...
 */
export const calendarTimeFormater = (unix) => dayjs.unix(unix).calendar(null, CALENDAR_FORMATTER);
